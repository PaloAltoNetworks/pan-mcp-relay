...
class TestPanSecurityRelay:
    ...
    @patch(
        "pan_aisecurity_mcp_relay.pan_security_relay.PanSecurityRelay._update_security_scanner", new_callable=AsyncMock
    )
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.PanSecurityRelay._load_config")
    async def test_initialize_success(
        self, mock_load_config, mock_update_security_scanner, mock_update_tool_registry, relay, valid_config
    ):
    ...
    async def test_initialize_config_missing_pan_security(self, mock_load_config, relay):
        ...
        config_missing_pan_security = {
            "benign_text_processor": {"command": "python", "args": ["/path/to/benign_text_processor.py"]},
            "benign_data_analyzer": {"command": "python", "args": ["/path/to/benign_data_analyzer.py"]},
        }
    ...
    @patch("pan_aisecurity_mcp_relay.configuration.Configuration.load_config")
    ...
    @patch("pan_aisecurity_mcp_relay.security_scanner.SecurityScanner")
    async def test_update_security_scanner_success(
        self, mock_security_scanner, mock_cleanup, mock_initialize, relay, valid_config
    ):
    ...
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.PanSecurityRelay._disable_tools_with_duplicate_names")
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.PanSecurityRelay._validate_tool_limits")
    @patch(
        "pan_aisecurity_mcp_relay.pan_security_relay.PanSecurityRelay._collect_tools_from_servers",
        new_callable=AsyncMock,
    )
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.PanSecurityRelay._load_config")
    async def test_update_tool_registry_success(
        self, mock_load_config, mock_collect_tools, mock_validate_limits, mock_disable_duplicates, relay, valid_config
    ):
    ...
    @patch("pan_aisecurity_mcp_relay.downstream_mcp_client.DownstreamMcpClient.initialize", new_callable=AsyncMock)
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.PanSecurityRelay._prepare_tool", new_callable=AsyncMock)
    async def test_collect_tools_from_servers_success(
        self, mock_prepare_tool, mock_initialize, mock_list_tools, mock_cleanup, relay
    ):
    ...
    @patch("pan_aisecurity_mcp_relay.downstream_mcp_client.DownstreamMcpClient.list_tools", new_callable=AsyncMock)
    @patch("pan_aisecurity_mcp_relay.downstream_mcp_client.DownstreamMcpClient.initialize", new_callable=AsyncMock)
    async def test_collect_tools_from_servers_initialization_failure(
        self, mock_initialize, mock_list_tools, mock_cleanup, relay
    ):
    ...
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.stdio_server")
    ...
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.uvicorn.Server")
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.uvicorn.Config")
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.Starlette")
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.Route")
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.MessagesEndpoint")
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.SseEndpoint")
    @patch("pan_aisecurity_mcp_relay.pan_security_relay.SseServerTransport")
    ...
    async def test_run_sse_server_uvicorn_failure(
        self, mock_sse_transport, mock_starlette, mock_uvicorn_config, mock_uvicorn_server, relay
    ):
    ...
    async def test_handle_tool_execution_success(
        self, mock_scan_request, mock_scan_response, mock_should_block, mock_execute_on_server, relay
    ):
    ...
    async def test_handle_tool_execution_special_relay_info_tool(
        self, mock_scan_request, mock_should_block, mock_handle_relay_info, relay
    ):
        ...
        expected_result = types.CallToolResult(
            content=[
                types.TextContent(
                    type="text", text='{"servers": {"benign_text_processor": ["summarize_text_content"]}}'
                )
            ],
            isError=False,
        )
